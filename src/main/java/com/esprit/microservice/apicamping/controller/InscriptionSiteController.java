...
import com.esprit.microservice.apicamping.service.IInscriptionSiteService;
...
public class InscriptionSiteController {
    ...
    @GetMapping("/my-inscriptions/{utilisateurId}")
    public List<InscriptionSiteResponse> getMyInscriptions(@PathVariable Long utilisateurId) {
        return iInscriptionSiteService.getMyInscriptions(utilisateurId);
    }

    @Operation(description = "Récupérer les réservations des camps du guide connecté")
    @GetMapping("/my-camp-booking-list/{ownerId}")
    public List<InscriptionSiteResponse> getMyCampBookingList(@PathVariable Long ownerId) {
        return iInscriptionSiteService.getMyCampBookingList(ownerId);
    }
}